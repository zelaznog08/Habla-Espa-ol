...
// Memoized message item to prevent re-parsing markdown on every keystroke in parent
...
// Separate input component to isolate typing state and prevent full chat re-renders
...
export default function Chat() {
  ...
  const handleSend = useCallback(async (text: string) => {
    ...
    try {
      const response = await getSpanishResponse([...messages, userMsg]);
      setMessages(prev => [...prev, { role: "model", text: response }]);
    } catch (error) {
      console.error(error);
      setMessages(prev => [...prev, { role: "model", text: "Perdón, tuve un problema técnico. ¿Podemos intentar de nuevo?" }]);
    } finally {
      setIsLoading(false);
    }
  }, [messages, isLoading]);
